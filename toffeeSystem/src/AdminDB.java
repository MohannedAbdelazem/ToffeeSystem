...
public class AdminDB {
    ...
    AdminDB(){
        ...
        try{
            ...
            while(rs.next()){
                Admin a = new Admin(rs.getInt("ID"), rs.getString("name"),rs.getString("Password"), rs.getInt("age"),rs.getString("gender"), rs.getString("address"), rs.getString("email"));
                admins.add(a);
            }
    ...
    public void addNewAdmin(int id, String Name,String password, int age, String gender, String address,String email){
        Admin a = new Admin(id, Name, password,  age, gender, address, email);
        boolean flag = true;
        if(this.admins != null){
        ...
        if(flag){
            try {
                ...
                String SQL = "INSERT INTO Admin (ID, name,password, age, gender, address, email) VALUES(?, ?, ?, ?, ?, ?, ?)";
    ...
    public void DisplayAdmins(){
        ...
        System.out.printf("ID                  Name                Age               Gender            Address            Email\n");
        for(int i = 0;i<admins.size();i++){
            System.out.printf("%-20d%-20s%-20d%-20s%-20s%-20s\n", admins.get(i).getId(), admins.get(i).getName(), admins.get(i).getAge(), admins.get(i).getGender(), admins.get(i).getAddress(), admins.get(i).getEmail());
        }
    }
